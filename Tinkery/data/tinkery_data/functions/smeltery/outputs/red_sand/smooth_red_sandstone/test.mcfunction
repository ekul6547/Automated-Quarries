...
execute as @e[type=minecraft:item_frame,tag=t_v_current,limit=1] at @s if score #tinkery t_dum0 matches 720.. if block ~ ~-1 ~ minecraft:cauldron[level=3] if entity @e[type=minecraft:item_frame,nbt={Facing:1b},distance=..0.7,limit=1,nbt={Item:{id:"minecraft:paper",tag:{TinkeryCast:"dust"}}}] positioned ~ ~-0.5 ~ run function tinkery_data:smeltery/outputs/red_sand/smooth_red_sandstone/spawn
